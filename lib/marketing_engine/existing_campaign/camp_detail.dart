...
class CampaignDetailsPage extends StatelessWidget {
  final AssociatedTargetList _product;

  CampaignDetailsPage(this._product);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text("Targetlist detail"),
      ),
      body: SingleChildScrollView(
        child: Column(
          children: <Widget>[
            CampaignsTable(_provider.campDetailStream),
          ],
        ),
      ),
    );
...
class CampaignsTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<AssociatedTargetList>(
      stream: _product,
      builder: (_, snapshot) {
        if (snapshot.hasData && snapshot.data != null) {
          AssociatedTargetList detail = snapshot.data;
          return Table(
            children: [
              TableRow(
                children: [
              ...
              TableRow(
                children: [
                  ...
                  Padding(
                    padding: padding,
                    child: Text(channelValues.reverse[detail.channel]),
                  ),
                ],
              ),
              TableRow(
  ...
  final Stream<AssociatedTargetList> _product;

  CampaignsTable(this._product);
}
